...
int main() {
    ingresoDeDatos(); //nHab,dContag,linf,lsup,semilla,tFinal,tRec
        ...
        cout << "\nGeneracion de arreglos DONE";
        /*----------------------------------------*/
    asignRandomXY(puntero_positions,nHab);
    iniciaInfectadoRecupTime(puntero_positions,nHab);
    mostrarMatriz(puntero_positions,nHab,nCol);
            nRec=0;
            nInf=1;
            nSus=nHab-nInf; 
        /*----------------------------------------*/
        cout << "\n\nLa duracion del periodo es: " << tFinal; //>fout
        cout << "\nDia: " << time+1; //>fout
        cout << "\nSusceptibles es: " << nSus; //>fout
        cout << "\nInfectados es: " << nInf; //>fout
        cout << "\nRecuperados es: " << nRec << endl; //>fout
        /*--------Comienzo de la iteración---------*/
        aumentoTiempoRecuperados(puntero_positions,nHab,tRec);
            newRec=countRec(puntero_positions,nHab);
        cout << "\nLa cantidad de nuevos recuperados es: " << newRec << endl; //>fout
            newInf=newInfected(puntero_positions, nHab, dContag);
        cout << "\n\nLa cantidad de nuevos infectados es: " << newInf << endl; //>fout
            nRec=nRec+newRec;        
    ...
    while(time<tFinal && resp=="si") {
        ...
        movePos(puntero_positions, nHab, nCol);
            time++;
        cout << "\nDia: " << time+1 <<endl; //>fout
        cout << "\nLa cantidad de infectados es: " << nInf; //>fout
        cout << "\nLa cantidad de susceptibles es: " << nSus << endl; //>fout
        mostrarMatriz(puntero_positions,nHab,nCol);
        aumentoTiempoRecuperados(puntero_positions,nHab,tRec);
            newRec=countRec(puntero_positions,nHab);
        cout << "\nLa cantidad de nuevos recuperados es: " << newRec << endl; //>fout
